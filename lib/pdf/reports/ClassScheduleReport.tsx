...
export function ClassScheduleReport({ data, settings }: Props) {
  ...
  const targetClasses =
    settings.selectedIds.length > 0
      ? data.classes.filter((c) => settings.selectedIds.includes(c.id))
      : data.classes

  return (
    <Document>
      {targetClasses.map((cls) => (
        <Page key={cls.id} size="A4" orientation="landscape" style={pdfStyles.page}>
          <Text style={pdfStyles.title}>{cls.name} 時間割</Text>
